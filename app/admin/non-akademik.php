...
<div class="container">
    ...
    <?php
    $batas = 2;
    $halaman = isset($_GET['non-akademik'])?(int)$_GET['non-akademik'] : 1;
    ...
    if (isset($_POST['cari'])) {
		$cari = $_POST['cari'];
		$query = "SELECT * FROM register WHERE (nama LIKE '%".$cari."%' OR semester LIKE '%".$cari."%' OR ipk LIKE '%".$cari."%') AND beasiswa = 'Non Akademik' LIMIT $halaman_awal, $batas";				
	} else{
		$query = "SELECT * FROM register WHERE beasiswa = 'Non Akademik' LIMIT $halaman_awal, $batas";		
	}
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-none">
        <div class="container-fluid">
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    <li class="page-item">
                        <a class="page-link text-light bg-secondary" <?php if($halaman > 1){ echo "href='?non-akademik=$previous'"; } ?> aria-label="Previous">
                    ...
                    <li class="page-item">
                        <a class="page-link text-light bg-secondary" <?php if($halaman < $total_halaman) { echo "href='?non-akademik=$next'"; } ?> aria-label="Next">
